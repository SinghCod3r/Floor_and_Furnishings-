...
function ComplaintDetials() {
  ...
  const getCustomerDetailHandler = () => {
    GetDataWithToken(`superadmin/get-complaint-detail/${location?.state?.data?.id}`).then((response) => {
      if (response?.status) {
              ...
              <div className="col-xl-12">
                <div className="card">
                  <div className="card-header border-0 d-flex justify-content-between" >
                    ...
                    <div>
                      {CustomerDetials?.status !== "completed" && <Link className="btn btn-primary me-1" to="/AddInstalerSchdule"
                        state={{ ...CustomerDetials, category: location?.state?.data?.enquiry?.products }}>Assign Installer</Link>}
                      <button type="button" onClick={modalToggle2} className="btn btn-primary me-1">
                        Add Remark
                      </button>
                      {CustomerDetials?.status !== "completed" && < button type="button" onClick={modalToggle} className="btn btn-primary">
                        Complete Complaint
                      </button>}
                    </div>
                  </div>
                  ...
                  <div className="table-responsive">
                    ...
                    >
                      ...
                      <tbody>
                        {CustomerDetials?.complaint_info && JSON.parse(CustomerDetials?.complaint_info)?.map((item, index) =>
                          < tr >
                            <td>
                              {item?.complaintId}
                            </td>
